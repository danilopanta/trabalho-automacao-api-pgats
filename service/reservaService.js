...
class ReservaService {
    ...
    criarReserva(idHospede, idQuarto, dataCheckin, dataCheckout) {
        ...
        if (!hospedeService.verificarSeHospedeExiste(idHospede)) {
            throw new Error('Hóspede não encontrado');
        }

        // Verificar se quarto existe
        const quarto = quartoService.consultarQuartoPorId(idQuarto);

        // Verificar disponibilidade do quarto nas datas informadas
        if (!this.verificarDisponibilidadeNoPeriodo(idQuarto, dataCheckin, dataCheckout)) {
        ...
        quartoService.marcarComoOcupado(idQuarto);
